...
export default function ConversationDetails() {
  ...
  const { contactName, image, messageHistory,group_name } = conversation;
  ...
  function changeHandler(evt: KeyboardEvent<HTMLInputElement>) {
    ...
    if ( key ==="Enter") {
      const teste = { "me": true, "message": messageSend, "message_Time": "12:11", "message_day": "today", "from_name": "Bob", "is_replay": false, "replay_message": "" ,"message_type":"text"};
    ...
    <div className="flex flex-col w-full">
      <div className="flex justify-between w-full px-4">
        <div className="flex justify-between bg-[#202c33] w-full h-14">
          <div className="flex items-center gap-4 h-full">
            ...
            <p className="flex justify-end text-[hsla(0,0%,100%,0.6)] text-xs mt-1">{group_name}</p>
          </div>
      ...
      <div className="flex flex-col w-full h-full px-24 py-6 overflow-y-auto" style={{ backgroundImage: "url('/assets/images/background.jpg')" }}>
        {
          message.map( ( messageConversation, index ) => {
            const { me, message, message_Time, message_day, from_name, is_replay, replay_message, message_type } = messageConversation;

            return (
              <MessageBalloon key={index} me={me} message={message} message_Time={message_Time} message_day={message_day} from_name={from_name} is_replay={is_replay} replay_message={replay_message} message_type={message_type}/>
            )
          } )
        }
      </div>
      <footer className="flex items-center bg-[#202c33] w-full h-16 py-3 text-[#8696a0]">
        ...
        <div className="flex w-[85%] h-12 ml-3">
          <input type={"text"} className="bg-[#2a3942] rounded-lg w-full px-3 py-3 text-white" placeholder="Messaggio" onKeyDown={(evt) => changeHandler(evt) } onChange={ (evt) => setMessageSend(evt.target.value) } value={messageSend} />
        </div>
